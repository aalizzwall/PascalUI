...
local function setVengMax(currentVengeance)
	f.vengmax = UnitHealthMax("player")
...
local function formatTime(timenum)
	...
	if minutes > 0 then
		text = strformat("%d 分:%d 秒",minutes,seconds)
	elseif seconds > 0 then
		text = strformat("%d 秒",seconds)
	end
	return text == "" and "0" or text
end
...
local function vengReport(fightMax, fightMaxPercent, fightAverage, fightDowntime, fightTime)
	...
	if fightTime > 0 and fightTime >= fightDowntime then
		...
		tinsert(f.lastReport,{"持續/總時間：",uptimeText.."/"..fightTimeText})
		tinsert(f.lastReport,{"持續時間(%)：",strformat("%.2f%%",uptimePerc)})
...
--	border:SetTexture([[Interface\Tooltips\UI-StatusBar-Border]])
  ...
  function(self) 
  	...
  	GameTooltip:SetText("|TInterface\\Icons\\Ability_Paladin_ShieldofVengeance:16|t 憎恨統計", 1, 0.4, 0.4, 1)
