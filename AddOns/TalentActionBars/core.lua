	...
	args = {
		...
		ActiveSpells = {
			...
			set = function(_,v)	
				...
				if v then 
					for y = 1,18 do
						local name, texture, tier, column, selected = GetTalentInfo (y)
...
function addon:PLAYER_LOGIN (event, msg)
	for y = 1,18 do
		local name, texture, tier, column = GetTalentInfo (y)		
		addon.talents [name] = tier
		if addon.talents [tier] == nil then addon.talents [tier] = { } end
		addon.talents [tier][column] = name
	end
end
...
function addon:saveActiveSpells (tier) 
	for i = 1,120 do
		...
		if tpe == "spell" then
			...
			for col = 1, 3 do
				local name, texture, t, column, selected = GetTalentInfo ((tier - 1) * 3 + col)
