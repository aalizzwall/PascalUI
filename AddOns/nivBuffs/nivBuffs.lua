...
local defaults = {
	...
	remainingTimeFormat = "Abbreviated",	-- remaining time format possible values are : "HH:MM:SS", "Abbreviated"
	...
	formatTimeRemaining = function(timeLeft)
		...
		if addon.db.profile.remainingTimeFormat == "Abbreviated" then
			if (timeLeft >= 86400) then
				return ("%d天"):format(round(timeLeft / 86400))
			end
			if (timeLeft >= 3600) then
				return ("%d時"):format(round(timeLeft / 3600))
			end
			if (timeLeft >= 60) then
				return ("%d分"):format(round(timeLeft / 60))
			end
			return ("%d秒"):format(timeLeft)
