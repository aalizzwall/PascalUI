...
function JSHB.SetupAspectIndicatorModule()
	...
	if JSHB.db.profile.indicators.aspect_enable then
		...
		local function DoAspectUpdate(self)
			...
			elseif (not self.isRogue) and (UnitAura("player", "Aspect of the Cheetah") or UnitAura("player", "Aspect of the Pack")or UnitAura("player", "Aspect of the Fox")) then -- There is an aspect up
				--self._, self._name = GetShapeshiftFormInfo(GetShapeshiftForm() )
				--print("I'm here")
				if UnitAura("player", "Aspect of the Cheetah") then
					self._name = "Aspect of the Cheetah"
					--print("Test")
				end
				if UnitAura("player", "Aspect of the Pack") then
					self._name = "Aspect of the Pack"
				end
				if UnitAura("player", "Aspect of the Fox") then
					self._name = "Aspect of the Fox"
				end
				
				self._, self._, self._tex, self._, self._, self._ = GetSpellInfo(self._name)
			...
			if InCombatLockdown() then                                                                                              --or UnitAura("player", "Aspect of the Fox")
				if ( (PC == "HUNTER") and (UnitAura("player", "Aspect of the Cheetah") or UnitAura("player", "Aspect of the Pack") or UnitAura("player", "Aspect of the Fox")) ) -- missing or cheetah/pack
				...
				if ( (PC == "HUNTER") and (GetShapeshiftForm() ~= 1) ) -- missing or cheetah/pack
					...
					or ( (PC == "WARRIOR") and (GetSpecialization() ~= 3) and (GetShapeshiftForm() == 2) )
					or (self.isRogue and (not self.poisonTex) ) -- Check for Rogue Poisons!
					--(not JSHB.db.profile.indicators.aspect_onlycombat) and 
					and (not UnitInVehicle("player") ) and (not C_PetBattles.IsInBattle() ) then
				
					JSHB.F.AspectIndicator:SetAlpha( (JSHB.db.profile.indicators.aspect_matchbaralpha and JSHB.db.profile.energybar.enabled) and JSHB.F.EnergyBar:GetAlpha() or 1)
					AutoCastShine_AutoCastStop(JSHB.F.AspectIndicator.shine)
					
					--(not JSHB.db.profile.indicators.aspect_onlymissing) and (not JSHB.db.profile.indicators.aspect_onlycombat) and 
				elseif (not UnitInVehicle("player") )
