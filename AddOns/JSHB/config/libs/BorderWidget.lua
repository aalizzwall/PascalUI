...
do
	local widgetType = "LSM30_Border_JS"
	...
	local function ContentOnEnter(this, button)
		...
		local border = self.list[text] ~= text and self.list[text] or JSHB.GetActiveBorderFile(text, nil, "border")
	...
	local function SetList(self, list) -- Set the list of values for the dropdown (key => value pairs)
		...
		self.list = list or JSHB.MakeLSMHashTable(JSHB.defaultBorders, "border")
	end
