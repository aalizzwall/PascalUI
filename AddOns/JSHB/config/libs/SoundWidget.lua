...
local AGSMW = LibStub("AceGUISharedMediaWidgets-1.0-JS")

do
	local widgetType = "LSM30_Sound_JS"
	...
	local function ContentSpeakerOnClick(this, button)
		...
		--PlaySoundFile(self.list[sound] ~= sound and self.list[sound] or Media:Fetch('sound',sound), JSHB.db.profile.masteraudio and "Master" or nil)
		PlaySoundFile(self.list[sound] ~= sound and self.list[sound] or JSHB.GetActiveSoundFile(sound, nil, "sound"), JSHB.db.profile.masteraudio and "Master" or nil)
	end
	...
	local function SetList(self, list) -- Set the list of values for the dropdown (key => value pairs)
		...
		self.list = list or JSHB.MakeLSMHashTable(JSHB.defaultSounds, "sound")
	end
	...
	local function WidgetPlaySound(this)
		...
		--PlaySoundFile(self.list[sound] ~= sound and self.list[sound] or M-edia:Fetch('sound',sound), JSHB.db.profile.masteraudio and "Master" or nil)
		PlaySoundFile(self.list[sound] ~= sound and self.list[sound] or JSHB.GetActiveSoundFile(sound, nil, "sound"), JSHB.db.profile.masteraudio and "Master" or nil)
	end
