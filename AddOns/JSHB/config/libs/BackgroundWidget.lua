...
do
	local widgetType = "LSM30_Background_JS"
	...
	local function ContentOnEnter(this, button)
		...
		local background = self.list[text] ~= text and self.list[text] or JSHB.GetActiveBackgroundFile(text, nil, "background")
	...
	local function SetList(self, list) -- Set the list of values for the dropdown (key => value pairs)
		...
		self.list = list or JSHB.MakeLSMHashTable(JSHB.defaultBackgrounds, "background")
	end
