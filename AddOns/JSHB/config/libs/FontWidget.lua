...
do
	local widgetType = "LSM30_Font_JS"
	...
	local function SetList(self, list) -- Set the list of values for the dropdown (key => value pairs)
		...
		self.list = list or JSHB.MakeLSMHashTable(JSHB.defaultFonts, "font")
	end

	local function SetText(self, text) -- Set the text displayed in the box.
		self.frame.text:SetText(text or "")
		--local font = self.list[text] ~= text and self.list[text] or Media:Fetch('font',text)
		local font = self.list[text] ~= text and self.list[text] or JSHB.GetActiveFontFile(text, nil, "font")
	...
	local function ToggleDrop(this)
		...
		if self.dropdown then
			...
			for i, k in ipairs(sortedlist) do
				...
				local font = self.list[k] ~= k and self.list[k] or JSHB.GetActiveFontFile(k, nil, "font")
