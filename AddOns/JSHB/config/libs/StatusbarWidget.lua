...
do
	local widgetType = "LSM30_Statusbar_JS"
	...
	local function SetList(self, list) -- Set the list of values for the dropdown (key => value pairs)
		...
		self.list = list or JSHB.MakeLSMHashTable(JSHB.defaultTextures, "statusbar")
	end

	local function SetText(self, text) -- Set the text displayed in the box.
		self.frame.text:SetText(text or "")
		--local statusbar = self.list[text] ~= text and self.list[text] or Media:Fetch('statusbar',text)
		local statusbar = self.list[text] ~= text and self.list[text] or JSHB.GetActiveTextureFile(text, nil, "statusbar")
	...
	local function ToggleDrop(this)
		...
		if self.dropdown then
			...
			for i, k in ipairs(sortedlist) do
				...
				local statusbar = self.list[k] ~= k and self.list[k] or JSHB.GetActiveTextureFile(k, nil, "statusbar")
